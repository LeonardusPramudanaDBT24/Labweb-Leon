...
function App() {
  ...
  const loadBooks = async () => {
    try {
      ...
      const normalizedData = (val) => {
        const s = (val || "").toString().toLowerCase();
        if (/excel|excellent|Keren|great/.test(s)) return "excellent";
        if (/avg|average|biasa|b aja/.test(s)) return "average";
        if (/bad|kurang|kureng/.test(s)) return "bad";
        return "average";
      };
      ...
      const normalized = (Array.isArray(data) ? data : []).map((b) => ({
        ...
        rating: normalizedData(b.rating || b.rate || b.nilai || b.kualitas),
      }));
